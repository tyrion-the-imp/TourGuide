...
void IOTMFamiliarScrapbookGenerateResource(ChecklistEntry [int] resource_entries)
{
	...
	if (available_amount($item[familiar scrapbook]) > 0) {
		...
		resource_entries.listAppend(ChecklistEntryMake("__item familiar scrapbook", url, ChecklistSubentryMake(familiar_scraps / 100 + " scrapbook banishes available", "", description), -79).ChecklistEntrySetCombinationTag("banish").ChecklistEntrySetIDTag("Familiar scrapbook boring pictures banish"));
	}
}
