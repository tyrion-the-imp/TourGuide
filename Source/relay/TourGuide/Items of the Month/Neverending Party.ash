...
void IOTMNeverendingPartyGenerateTasks(ChecklistEntry [int] task_entries, ChecklistEntry [int] optional_task_entries, ChecklistEntry [int] future_task_entries)
{
    ...
    if (quest_state.in_progress) {
		...
		description.listAppend(HTMLGenerateSpanFont("["+$item[van key].to_int()+"] van key ("+available_amount($item[van key])+") |* food, burnout | ["+$item[unremarkable duffel bag].to_int()+"] unremarkable duffel bag ("+available_amount($item[unremarkable duffel bag])+") |*  booze, jock", "gray"));
