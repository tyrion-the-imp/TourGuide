...
void IOTMSITCertificateGenerateTasks(ChecklistEntry [int] task_entries, ChecklistEntry [int] optional_task_entries, ChecklistEntry [int] future_task_entries) {
	...
	string mySkillToday = "";
	foreach i in skillNames {
        if (lookupSkill("Psychogeologist").have_skill())    mySkillToday = "<font color=red><b>Psychogeology</b></font>";
    ...
    if (hasAnySkillOf(skillNames)) {
        ...
        if (!get_property_boolean("_sitCourseCompleted")) {
            ...
            main_title = "S.I.T. today - "+mySkillToday+"";
            optional_task_entries.listAppend(ChecklistEntryMake("__item S.I.T. Course Completion Certificate", url, ChecklistSubentryMake(main_title, subtitle, description), 0).ChecklistEntrySetIDTag("S.I.T. Course Completion Certificate"));
        }
    }
    else {
