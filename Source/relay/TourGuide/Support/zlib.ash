...
boolean vprint(string message, string color, int level) {
   if (level == 0) abort(message+" (0 verbosity ABORT)");
...
boolean vprint_html(string message, int level) {
   if (level == 0) { print_html(message); abort("(0 verbosity ABORT)"); }
...
boolean load_current_map(string fname, aggregate dest) {
   file_to_map(fname+".txt",dest); #return true;
...
boolean auto_mcd(int safemox) {
   if (!to_boolean(getvar("automcd")) || my_ascensions() < 1 || in_bad_moon() || my_path() == "Kingdom of Exploathing") return true;
